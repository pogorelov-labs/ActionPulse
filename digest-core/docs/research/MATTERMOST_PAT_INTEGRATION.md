...
### Live probe findings (2026-06-17, from outside the corp network)

A read-only probe of the live server was attempted from a dev box. Result — the most important operational fact for this integration:

- **The server is reachable, but the authenticated REST API is corp-network-only.** `GET /api/v4/system/ping` and the *unauthenticated* `GET /api/v4/users/me` / `/teams` all reach Mattermost (the latter return a Mattermost `401` JSON — the API path is open). The moment the same request carries `Authorization: Bearer <PAT>`, the corp **edge proxy returns a generic `403` (`nginx`, `text/html`) — the request never reaches Mattermost.** The only difference between the `401`-JSON and the `403`-nginx is the Bearer header, so the edge is **blocking token-authenticated API calls from outside** (browser cookie sessions and public endpoints are allowed; programmatic Bearer auth is not).
- **Consequence:** the PAT/REST integration (mm-source, reaction read, API delivery) follows ADR-012 **"code outside, run inside" exactly like EWS and the LLM gateway** — it must be developed and tested **from inside the corp network**. This narrows the earlier capability-study assumption that the MM API is "reachable from everywhere": only the **incoming webhook** (URL-embedded secret, no Bearer header — a separate path) is externally reachable; the authenticated API is not.
