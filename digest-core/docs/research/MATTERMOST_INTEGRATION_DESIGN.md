...
> **SHIPPED (P3, 2026-06-18).** The ladder is a single enum `mm_source.dm_scope ∈ {off, own_posts_only, selected, all}` (default `off`) + `dm_allowlist` + `dm_consent_acknowledged` (+ `_at`), the repo's **first consent primitive**. *(A per-post counterparty quote cap `dm_max_quote_chars` shipped here too, but was removed 2026-06-21 — DMs are now harvested in full; the store keeps DM bodies redacted at rest, guardrail #9.)* Decisions made during the build, each verified by tests + an adversarial review (all 10 privacy invariants hold):
> - **The DM allowlist matches the COUNTERPARTY's identity** (`user_id`/`@username`/email), **not** the channel id/name/display_name the channel allowlist uses — a `D`/`G` channel has no human-readable name (a `D` channel's name is `id1__id2`; `G` members come from `GET /channels/{id}/members`). Reusing `_channel_is_allowlisted` for DMs would silently match nobody.
> - **`own_posts_only` needs NO consent** (it drops every counterparty post before the LLM — no third party to consent for); only `selected`/`all` require the dated ack. This is what makes the "easy on/off" genuinely easy.
> - **Group-DMs (type `G`) follow the same `dm_scope`** as 1:1 DMs; under `selected` a group-DM is kept iff **ANY** non-owner member is allowlisted (owner's decision).
> - **The consent gate is load-bearing on LOAD** — `Config` reconstructs `MattermostSourceConfig` after the YAML merge (because `_merge_model` setattrs, skipping the `mode="after"` validator), so a hand-edited `config.yaml` cannot smuggle `selected`/`all` past the ack; the menu's `update_mm_source_dm` refuses to *persist* an unloadable state.
> - **Leak closed:** under `off`, `D`/`G` channels are never paged — an `@owner` mention inside a group-DM (which the mentions slice would otherwise ingest) now yields zero messages.
> - **Member-gate is before the content GET** (resolved in the coordinator from metadata only); **counterparty text is quote-capped** in `selected`/`all`, the owner's own posts uncapped; **at rest**, `--dump-ingest` redacts DM (`D`/`G`) bodies (the `NormalizedMessage.mm_channel_type` carrier drives it). Consent re-fires on widening/escalation/staleness (180 d), never on a partner-list edit or a downgrade. Blank `owner_id` fails closed (DMs disabled).
> - **Wizard + settings menu** expose the ladder (single-select with `isatty` degradation) + a consent panel; `actionpulse → Mattermost DMs` is the first in-menu config editor.
...
| **P3 ✅ SHIPPED (2026-06-18)** | **DMs ingest (consent-gated)** + wizard/menu consent UX | The most action-dense surface | done | Adapter live-gated (offline-built+tested) | DONE: 4-rung `dm_scope` ladder, counterparty-identity allowlist, group-DM ANY-match, load-bearing consent gate, leak closed, quote cap + at-rest dump-redaction. See §2.2 "SHIPPED". |
