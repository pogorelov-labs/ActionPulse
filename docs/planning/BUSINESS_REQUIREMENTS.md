...
- **Приватность и ИБ:** минимизация ПДн, управление согласиями для DM, хранение ≤ 7 дней.
    ...
    - _Carve-out — опциональное зашифрованное хранилище:_ при явном включении
      (`store.enabled`, по умолчанию OFF) сообщения архивируются в **SQLCipher-
      зашифрованном** хранилище на **30 дней** (`store.ttl_days`) для будущих
      lookup/поиска/векторизации. Более длинное окно допустимо именно потому, что
      данные зашифрованы at-rest (в отличие от plaintext-артефактов в `var/out`,
      которые остаются ≤ 7 дней). Тела DM (`D`/`G`) НИКОГДА не сохраняются
      (guardrail #9 — редактируются). Ключ только в ENV (`DIGEST_STORE_KEY`).
